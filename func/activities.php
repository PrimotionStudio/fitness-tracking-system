...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $weight = $get_user['weight'];

    $bmr = 10 * $weight;

    $calories_burned = (floatval($workout) * floatval($bmr) * floatval($intensity)) / (60 * floatval($duration));

    $insert_activity_metrics = "INSERT INTO activity_metrics (user_id, workout_type, weight, intensity, duration, calories_burned) VALUES ('$user_id', '$workout_type', $weight, $intensity, $duration, $calories_burned)";
